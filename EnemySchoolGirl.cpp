...
{	
	/*if (getDistance(_SgCenter.x, _SgCenter.y, _ptMouse.x,_ptMouse.y))
	{
		_ChaseAngle = getAngle(_SgCenter.x, _SgCenter.y, _ptMouse.x, _ptMouse.y);
		_SgCenter.x += cosf(_ChaseAngle) * _Speed;
		_SgCenter.y += -sinf(_ChaseAngle) * _Speed;
	}*/

	
	if (getDistance(_SgCenter.x, _SgCenter.y, _Player->GetShadowCenterX(), _Player->GetShadowCenterY()))
	{
		_ChaseAngle = getAngle(_SgCenter.x, _SgCenter.y, _Player->GetShadowCenterX(), _Player->GetShadowCenterY());
		_SgCenter.x += cosf(_ChaseAngle) * _Speed;
		_SgCenter.y += -sinf(_ChaseAngle) * _Speed;
	}
...
void EnemySchoolGirl::SgAniInit()
{
	...
	KEYANIMANAGER->addArrayFrameAnimation("SgRDazed", "SgDazed", Sg_R_Dazed, 4, 10, true);
	int Sg_L_Dazed[] = { 7,6,5,4 };
	KEYANIMANAGER->addArrayFrameAnimation("SgLDazed", "SgDazed", Sg_L_Dazed, 4, 10, true);
	...
	KEYANIMANAGER->addArrayFrameAnimation("SgRHoldhit", "SgHold", Sg_R_holdHit, 3, 10, true);
	int Sg_L_holdHit[] = {14,11,9};
	KEYANIMANAGER->addArrayFrameAnimation("SgLHoldhit", "SgHold", Sg_L_holdHit, 3, 10, true);

	int Sg_R_holdRe[] = { 3,5,7,0 };
	KEYANIMANAGER->addArrayFrameAnimation("SgRHoldrelrase", "SgHold", Sg_R_holdRe, 4, 10, true);
	int Sg_L_holdRe[] = { 12,10,8,15 };
	KEYANIMANAGER->addArrayFrameAnimation("SgLHoldrelrase", "SgHold", Sg_L_holdRe, 4, 10, true);
	...
	//여학생 무기로 쓸떄
	IMAGEMANAGER->addFrameImage("SgWeaponSwing", "image/enemy/sg/sg_weapon_swing2.bmp", 0, 0, 2646, 456, 6, 2, true, RGB(255, 0, 255));
	int Sg_R_Weapon[] = {0,1,2,3,4,5};
	KEYANIMANAGER->addArrayFrameAnimation("SgRWeaponSwing", "SgWeaponSwing", Sg_R_Weapon, 6, 10, true);
	int Sg_L_Weapon[] = { 11,10,9,8,7,6 };
	KEYANIMANAGER->addArrayFrameAnimation("SgLWeaponSwing", "SgWeaponSwing", Sg_L_Weapon, 6, 10, true);
